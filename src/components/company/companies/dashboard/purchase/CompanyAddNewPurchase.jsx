...
import CompanySuccessPurchase from './CompanySuccessPurchase';
import axios from 'axios';
import { toast, ToastContainer } from 'react-toastify';

const CompanyAddNewPurchase = ({ setClicked }) => {
    ...
    const [rows, setRows] = useState([
        { id: 1, stockName: 0, stockPrice: '', qty: '', totalPrice: 0 }
    ]);
    ...
    const handlePriceChange = (e, rowId) => {
        const newPrice = e.target.value; // Get the updated value from input
        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === rowId ? { ...row, stockPrice: newPrice } : row
            )
        );
    };
    ...
    const handleStockSelect = (rowId, stockId) => {
        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === rowId
    ...
    // sum up all total
    const totalAmount = rows.reduce((acc, row) => acc + (row.stockPrice * row.qty || 0), 0);
    ...
    const handleAddPurchase = () => {
        setBtn(true)
        axios.post('https://aamsheiliagunicorn-sms-wsgi-application.onrender.com/company/customer-purchase/', {
            items: rows.map(row => ({
                company_stock: row.stockId,
                unit_quantity: row.qty,
                unit_price: row.stockPrice,
    ...
    return (
        <div className='bg-color-full px-4 sm:px-0'>
            ...
            <div className='mt-10 grid grid-cols-1 sm:grid-cols-3 gap-8'>
                <div>
                    <p className='font-mont text-sm font-normal mb-2'>Customer's Name</p>
                        ...
                        <div key={row.id} className='flex justify-between my-1 bg-white/[0.47]'>
                            ...
                            <span className='py-2 px-4'>
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        handleStockSelect(row.id, stockId); // Call the function
                                    }}
                                >
                            ...
                            <span className='text-xs sm:text-sm font-mont py-2'>
                                <input
                                    ...
                                    onChange={(e) => handlePriceChange(e, row.id)} // Add this onChange
                                />
                            </span>
                            <span className='text-xs sm:text-sm font-mont py-2'>
                                <input
                            ...
                            <span value={row.totalPrice} className='text-xs sm:text-sm font-mont py-4 px-2'>₦{Number(row.stockPrice * row.qty).toLocaleString()}.00</span>
            ...
            {/* totals */}
            <div className='mt-20'>
                <div className='grid grid-cols-1 gap-0.5 mb-3 sm:mb-7'>
                ...
                alert && (
                    <div className='center-proceed'>
                        ...
                        <div className="center-content-proceed">
                            <CompanySuccessPurchase handleAlert={handleAlert} setClicked={setClicked} />
                        </div>
                    </div>
                )
            }
...
export default CompanyAddNewPurchase
