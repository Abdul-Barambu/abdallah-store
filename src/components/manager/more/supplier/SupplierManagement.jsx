    ...
    return (
        <div className='bg-color-dash mx-4'>
            ...
            <div className='mt-5 block sm:flex items-center justify-between'>
                ...
                {/* button */}
                <div className='bg-black py-3 sm:py-2 rounded-xl w-full sm:w-1/4 text-center cursor-pointer'>
                    ...
                    <div className='h-96 overflow-y-scroll'>
                        {
                            filteredDues.map((list) => (
                                <div key={list.id} className='grid grid-cols-6 my-0.5 text-center'>
                                    ...
                                    </div>                                    <div className='flex flex-row gap-4 justify-center items-center bg-white/[0.47]'>
                                        <IoEye className='cursor-pointer' onClick={() => { setClicked(); localStorage.setItem("ListOfStocks", JSON.stringify(list)) }} />
                                        <MdEditSquare className='cursor-pointer icon-blue' onClick={() => { setClicked(); localStorage.setItem("ListOfStocks", JSON.stringify(list)) }} />
